...
`
	...
	tests := []struct {
		...
		},
		{
			...
			args:        []string{"--inspect=9229", "./node_nodules/script.js"},
			env:         env{"WRAPPER_ALLOWED": "script.js"},
			expected:    "--inspect=9229\n./node_nodules/script.js\n",
		},
		{
		...
		},
		{
			...
			args:        []string{"node_modules/script.js"},
			env:         env{"NODE_DEBUG": "--inspect", "NODE_OPTIONS": "--inspect-brk"},
			expected:    "NODE_DEBUG=--inspect\nnode_modules/script.js\n",
		},
		{
	...
	for _, test := range tests {
		t.Run(test.description, func(t *testing.T) {
			env := env{"PATH": firstNodeDir + string(os.PathListSeparator) + actualNodeDir + string(os.PathListSeparator) + os.Getenv("PATH")}
